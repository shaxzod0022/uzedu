...
function ContactOptions({ data }) {
  return (
    <div className="flex items-center justify-center flex-wrap mt-10 max-w-[540px] md:max-w-[720px] lg:max-w-[960px] w-full mx-auto">
      {data.map((item) => (
        <div key={item.id} className="w-[300px] h-[350px] m-1.5">
          <a
            href={item.link}
            target="_blank"
          ...
          >
            <div className="flex flex-col items-center justify-center h-full py-10 px-[50px]">
              ...
              />
              <h2 className="text-base text-[#47495a] font-bold uppercase mt-10 text-center">
                {item.title}
              </h2>
              <p className="text-[#6a7c92] text-sm text-center mt-2.5">
                {item.disp}
              </p>
            </div>
          </a>
