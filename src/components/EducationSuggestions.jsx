...
function EducationSuggestions() {
    ...
    const errorLength = (title) => {
        return (
            <>
                <span className="absolute text-xl font-bold text-red-600 top-2 right-2"><CircleAlert /></span>
    ...
    const renderInput = (type, name, placeholder, extraProps = {}) => (
        <div className='relative mb-7'>
            <input
                ...
                className={`px-4 py-3 text-base border ${errorValue[name] ? 'border-red-500' : 'border-[#151d41]'} text-[#495057] bg-white rounded w-full focus:outline-[#28a745]`}
    ...
    const renderSelect = (name, placeholder, options = [], labelKey = "title", valueKey = "value") => (
        <>
            <select
                name={name}
                value={value[name]}
                onChange={handleInputChange}
                className={`px-3 py-4 text-base border ${errorValue[name] ? 'border-red-500' : 'border-[#151d41]'} text-[#495057] bg-white rounded w-full focus:outline-[#28a745]`}
            >
                ...
                <form onSubmit={handleSubmit} className='flex flex-col flex-wrap items-center w-full md:flex-row py-7'>
                    ...
                    <div className='w-full lg:w-1/2 lg:pl-2'>
                        <div className='flex flex-wrap gap-y-4 mb-7'>
                            ...
                            <div className='relative w-full sm:w-1/2 sm:pl-2'>
                                {region?.tumanlar ? renderSelect("district", "Tuman", region.tumanlar, "sub__title", "value") : renderSelect("district", "Tuman", [], "sub__title", "value")}
                            </div>
                        </div>
                        <div className='relative mb-7'>
                            {renderSelect('direction', "Yo'nalishni tanlang", directions, "title", "value")}
                        </div>
                    </div>
                    <div className='relative w-full mb-7'>
                        <textarea
                            ...
                            className={`p-4 text-base border ${errorValue.suggestions ? 'border-red-500' : 'border-[#151d41]'} text-[#495057] bg-white rounded w-full focus:outline-[#28a745]`}
                        ></textarea>
                        {
